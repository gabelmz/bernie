...
export function TemplateDataPipelineNode({ data, id }: NodeProps & { data: any }) {
  return (
    <div className="min-w-[240px] font-sans">
      ...
      <NodeWrapper id={id} data={data}>
        <NodeHeader 
          title={data.title || 'Data Pipeline'} 
          icon={<Network className="w-4 h-4 text-text-muted" />} 
          badge="template_data_pipeline"
          backgroundColor={data.backgroundColor}
        />
        <div className="p-3 text-[12px] text-text-muted flex flex-col gap-2">
           <div className="bg-violet-500/10 border-violet-500/30 p-2 rounded flex items-center justify-center border">
